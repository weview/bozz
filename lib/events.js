...
const Events = function (ms) {
  ...
  this.isReady          = false;
  this.listeners        = [];
  this.waitingUnwatches = [];
};

Events.prototype.init = function () {
  const ms = this.ms;
  return ms.channel.assertExchange(ms.exEvents, 'fanout', { durable : false })
  .then(() => ms.channel.assertQueue(ms.queueEvents, { durable : false }))
  .then(() => ms.channel.consume(ms.queueEvents, msg => {
    this.handleEvent(msg)
  }, { noAck : true }))
  .then(() => (this.isReady = true))
  .then(() => this.proceedWaitingUnwatch())
  .then(() => this.proceedWaitingWatch());
};
...
Events.prototype.proceedWaitingUnwatch = function () {
  return Promise.all(
    this.waitingUnwatches.map((key) => this.unwatch(key))
  );
};

Events.prototype.proceedWaitingWatch = function () {
  const ms = this.ms;
  return Promise.all(
...
Events.prototype.unwatch = function (key) {
  const ms = this.ms;

  if (!this.isReady) {
    return this.waitingUnwatches.push(key);
  }

  return ms.channel.unbindQueue(ms.queueEvents, ms.exEvents, key);
};
...
Events.prototype.watch = function (key, method) {
  ...
  try {
    ms.channel.bindQueue(ms.queueEvents, ms.exEvents, key);
    this.ms.registry.registerEvent(key, method);
  } catch (e) {
    return Promise.reject(e);
  }
